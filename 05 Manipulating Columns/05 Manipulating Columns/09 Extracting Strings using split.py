...
df.split(lit("Hello World, how are you"), " "). \
    show(truncate=False)

# COMMAND ----------

df=df.select(split(lit('helo,world,dd'), ",")).show()
...
employeesDF.select(split("phone_number", " ")).show()
...
df.select(split(lit("Shbham,chavan,Family"),",")).show()
