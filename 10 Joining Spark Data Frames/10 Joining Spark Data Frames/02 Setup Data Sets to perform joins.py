...
users_df.alias('u').join(course_enrolments_df.alias('e'),users_df.user_id==course_enrolments_df.user_id).groupBy('u.user_id').count().show()

# COMMAND ----------

users_df.alias('u').join(course_enrolments_df.alias('e'),users_df.user_id==course_enrolments_df.user_id).groupBy(users_df['user_id']).count().show()

# COMMAND ----------

users_df.alias('u').join(course_enrolments_df.alias('e'),'user_id').groupBy('user_id').count().show()
...
users_df.alias('u').join(course_enrolments_df.alias('e'),'user_id','left').select('u.*','e.course_id','e.course_enrolment_id').\
filter('e.course_enrolment_id is NULL').show()
...
course_enrolments_df.alias('u').join(users_df.alias('ce'), users_df.user_id == course_enrolments_df.user_id, 'left').count()

# COMMAND ----------

users_df.alias('c').join(course_enrolments_df.alias('ce'),users_df.user_id == course_enrolments_df.user_id,'left').\
groupBy('user_first_name').count().\
select("*").show()

# COMMAND ----------

courses_df.alias("c").select('c.course_title').show()
...
courses_df.alias("ce").filter('ce.is_active=True').show()
