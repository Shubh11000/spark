...
df.select(to_date(lit('10092022'),'ddMMyyyy').alias('todate')).show()
...
datetimesDF.select(to_timestamp(col('time'),'dd-MMM-yyyy HH:mm:ss.SSS')).show()
...
datetimesDF.select(to_date(col('date').cast('string'),'yyyyMMdd').alias('to_date')).show()
